  ...
  try {
    ...
    const healthData: any = {
      ...
      checks: {
        ...
        serverListening: true, // We're responding, so yes!
      },
      memory: {
    ...
    // Include build errors if requested
    ...
    // This keeps health checks passing during startup
    ...
    // CRITICAL: Even if error reading state, return 200 to keep health checks passing
    ...
    return NextResponse.json({
      ...
      status: 200, // Always 200 unless critical failure
    });
  }
}

/**
 * Parse build/typecheck errors from logs
 */
function parseBuildErrors(logContent: string): ParsedError[] {
  ...
  // Parse ESLint errors: line:col  error  message  rule
